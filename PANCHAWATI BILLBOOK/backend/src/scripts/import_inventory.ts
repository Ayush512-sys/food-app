...
async function main() {
  ...
  if (!admin) {
    admin = await prisma.user.create({
      data: { email: 'admin@billbook.com', name: 'Admin', password_hash: '$2b$10$vukyB.hwzmgA32GEIsXMb.8QUHb5j.7BuzS.K6GUflYZdfGMY9SY2', role: 'admin' },
    });
  }

  // 3. Extract unique categories and create them
  console.log('Extracting and creating categories...');
  const uniqueCategories = Array.from(new Set(items.map((item: any) => item.category || 'Uncategorized')));
  
  const categoryMap = new Map<string, number>();
  for (const catName of uniqueCategories) {
    let category = await prisma.category.findUnique({ where: { name: catName as string } });
    if (!category) {
      category = await prisma.category.create({ data: { name: catName as string } });
    }
    categoryMap.set(catName as string, category.id);
  }
